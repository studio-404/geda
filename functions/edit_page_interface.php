...
class edit_page_interface extends connection{
	...
	public function general_form_news($fetch){
		...
		if($fetch["sma_page_type"]=="eventpage"){ // if eventpage
			...
			$out .= '<label for="event_website">Event website: </label>';
	...
	public function content_form($fetch){
		...
		$out .= '<label>Left side: </label>';
		$out .= '<textarea name="left_side" class="tinyMce">'.htmlentities($fetch['leftside']).'</textarea>';
		$out .= '<label>Right side: </label>';
		$out .= '<textarea name="right_side" class="tinyMce">'.htmlentities($fetch['rightside']).'</textarea>';
		return $out;
	}
