			...
			$o = '<ul class="nav navbar-nav">';
				for($x=0;$x<count($menu_array->date);$x++){
					...
					if($menu_array->sub[$x]){ 
						...
						$o .= '<a href="'.MAIN_DIR.$menu_array->slug[$x].'" class="sub_menu_arrow dropdown-toggle" data-toggle="dropdown">'.$menu_array->title[$x].'</a>';
						$o .= $this->sub($menu_array->sub[$x],$slug,"header"); 
						$o .= '</li>'; 
						
					}else{
						$o .= '<li class="'.$active.'"><a href="'.MAIN_DIR.$menu_array->slug[$x].'">'.$menu_array->title[$x].'</a></li>'; 
					}
				}			
			$o .= '</ul>';
		}else if($type=="footer"){
			...
			$o = '<div class="gmenu g'.LANG.'"><ul>';
				for($x=0;$x<count($menu_array->date);$x++){
					...
					if($menu_array->sub[$x]){ 
						...
						$o .= '<li><a href="'.MAIN_DIR.$menu_array->slug[$x].'" class="'.$active.'">'.$menu_array->title[$x].'</a></li>'; 
					}
				}			
			$o .= '</ul>';
		}
		...
		if($type=="header"){
			...
			for($x=0;$x<count($sub->date);$x++){
				$active = ($sub->slug[$x]==$slug) ? 'gsubactive' : '';
					$o .= '<li><a href="'.MAIN_DIR.$sub->slug[$x].'" class="'.$active.'">'.$sub->title[$x].'</a></li>'; 
			}
			$o .= '</ul>';
		}
		return $o;
